...
> **注意**：所有成功响应必须包含 `code`、`message`、`data` 三个字段。当前部分接口仅返回 `{code, data}`，Phase 2 起统一补齐 `message` 字段。
...
> TODO: [待补充] API 版本控制策略（URL 前缀 /v1/ 或 Header 版本号）。
> TODO: [待补充] 分页规范 — 当前未统一分页参数格式，建议统一为 `{ "items": [], "total": N, "page": N, "page_size": N }`。
